...
Блоки коммуницируют слоями. View-Model-DataLake, где DataLake это DataBase и Data-Source.

 #### View (сайт)
 
 ![alt text](ViewMain.png "ViewMain")
 
 Сайт сделан с помощью стандартных фреймворков, от пользователя требуется только авторизация и данные по поиску. Сайт также используется для приобретения премиум-статуса. На БД запросы посылаются на JS языке.
...
Взяли основную структуру, и многослойность.
...
![alt text](Reference.png "Reference")
 ...
 #### F-4. Система должна создавать две очереди запросов на анализ пользователей: очередь пользователей, обладающих премиум аккаунтом, рассмотривается раньше, чем пользователей с базовым аккаунтом

_______

 ### Нефункциональные требования 
 #### NF-1. Система должна проводить поиск и анализ кандидатов от 5 до 20 минут(в зависимости от указанной глубины поиска)
